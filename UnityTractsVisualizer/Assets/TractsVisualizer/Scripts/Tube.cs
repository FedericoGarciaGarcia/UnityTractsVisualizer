...
public class Tube
{
	...
	public Color[] colors;   // Color by vertex
	...
	void GenerateColors() {
		for(int i=0; i<polyline.Length-1; i++) {
			Vector3 v = polyline[i+1]-polyline[i];
			v = Vector3.Normalize(v);
			
			for(int j=0; j<resolution; j++) {
				colors[i*resolution+j] = new Color(Mathf.Abs(v.x*v.x), Mathf.Abs(v.y*v.y), Mathf.Abs(v.z*v.z));
			}
		}
		
		for(int j=0; j<resolution; j++) {
			colors[(polyline.Length-1)*resolution+j] = colors[(polyline.Length-2)*resolution+j];
		}
	}
}
